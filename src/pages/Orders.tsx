...
export default function Orders() {
  ...
  const checkout = useMutation({
    ...
    onError: () => toast({ title: "Pedido não criado", description: "Revise os itens e tente novamente.", variant: "destructive" }),
  });
  ...
  return <div className="min-h-screen bg-background p-4 md:p-8 lg:p-10"><div className="mx-auto max-w-7xl space-y-7">
    ...
    <Tabs defaultValue="new"><TabsList className="mb-7 grid h-auto w-full grid-cols-2 rounded-2xl bg-muted p-1 sm:w-fit"><TabsTrigger value="new" className="rounded-xl">Novo pedido</TabsTrigger><TabsTrigger value="history" className="rounded-xl">Histórico ({orders.data?.length ?? 0})</TabsTrigger></TabsList>
      <TabsContent value="new"><div className="grid gap-5 xl:grid-cols-[1fr_380px]">
        ...
        <Card className="surface-elevated h-fit rounded-2xl xl:sticky xl:top-6"><CardHeader><CardTitle className="flex items-center gap-2 text-xl"><ShoppingBag className="h-5 w-5 text-primary"/>Resumo do pedido</CardTitle></CardHeader><CardContent className="space-y-5"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1"><div className="space-y-2"><Label>Canal</Label><Select value={channel} onValueChange={setChannel}><SelectTrigger className="rounded-xl"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="counter">Balcão</SelectItem><SelectItem value="pickup">Retirada</SelectItem><SelectItem value="delivery">Delivery</SelectItem><SelectItem value="whatsapp">WhatsApp</SelectItem></SelectContent></Select></div><div className="space-y-2"><Label>Cliente</Label><Select value={customerId} onValueChange={setCustomerId}><SelectTrigger className="rounded-xl"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="none">Consumidor não identificado</SelectItem>{customers.data?.map(customer => <SelectItem key={customer.id} value={customer.id}>{customer.name} · {customer.phone}</SelectItem>)}</SelectContent></Select></div></div><div className="max-h-72 space-y-2 overflow-y-auto">{cart.length ? cart.map(item => <div key={item.id} className="flex items-center gap-3 rounded-xl bg-muted/60 p-3"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{item.name}</p><p className="text-xs text-muted-foreground">{money.format(Number(item.sale_price) * item.quantity)}</p></div><div className="flex items-center gap-1"><Button type="button" size="icon" variant="ghost" className="h-7 w-7" onClick={() => changeQuantity(item.id,-1)}><Minus className="h-3.5 w-3"/></Button><span className="w-5 text-center text-sm">{item.quantity}</span><Button type="button" size="icon" variant="ghost" className="h-7 w-7" onClick={() => changeQuantity(item.id,1)}><Plus className="h-3.5 w-3"/></Button><Button type="button" size="icon" variant="ghost" className="h-7 w-7 text-destructive" onClick={() => setCart(current => current.filter(product => product.id !== item.id))}><Trash2 className="h-3.5 w-3"/></Button></div></div>) : <p className="py-10 text-center text-sm text-muted-foreground">Selecione produtos para começar.</p>}</div><div className="flex items-center justify-between border-t pt-4"><span className="font-medium">Total</span><strong className="text-2xl text-primary">{money.format(total)}</strong></div><Button className="h-12 w-full rounded-xl" disabled={!cart.length || checkout.isPending} onClick={() => checkout.mutate()}>{checkout.isPending ? "Criando pedido…" : "Confirmar pedido"}</Button></CardContent></Card>
      </div></TabsContent>
